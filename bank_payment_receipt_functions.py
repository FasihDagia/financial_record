...
class bank_pay_rece:
    ...
    def generate_bank_payments(self,window,payments_temp,payment,pay_receip,pay_receip_temp,customers,client_temp,bank,bank_temp,banks,bank_ind_temp,tax,tax_temp,invoice_balance,heads,company_name,user_name,db,invoice_temp,head_collection,head_temp):
        
        for widget in self.root.winfo_children():
            widget.destroy()

        self.root.title("Generate Payment")
        ...
        ttk.Label(self.root,text="Generate Bank Payment Voucher",font=("helvetica",18,"bold")).pack(pady=30)
        ...
        ttk.Button(self.root,text="Generate" ,style="Module.TButton",cursor="hand2",width=20,command=lambda:generate(self.root,window,payments_temp,payment,pay_receip,pay_receip_temp,customers,client_temp,bank,bank_temp,banks,bank_ind_temp,tax,tax_temp,invoice_temp,head_collection,head_temp)).pack(pady=10)    
        
        btn_frame = tk.Frame(self.root) 
        btn_frame.pack()

        ttk.Button(btn_frame,text="Back" ,style="Logout.TButton",cursor="hand2",width=10,command=lambda:window(self.root,company_name,user_name)).grid(row=0,column=0,padx=5)
        ttk.Button(btn_frame,text="Exit" ,style="Logout.TButton",cursor="hand2",width=10,command=self.root.destroy).grid(row=0,column=1,padx=5)
        ...
        def generate(root,window,payments_temp,payment,pay_receip,pay_receip_temp,customers,client_temp,bank,bank_temp,indvidual_bank,bank_ind_temp,tax,tax_temp,invoice_temp,head_collection,head_temp):
            ...
            if not date or not vouch_no or not account or not acc_recev or not exp_type or not description  or not amount or not tax_percent or not tax_amount or not total_amount:
                ...
                self.records(payments_temp,payment,total_amount,"add",date,vouch_no,invoice_no,cheque_no,exp_type,account,acc_recev,description,amount,amountiw,tax_percent,tax_amount,total_amount)

                #for overall bank and cash record
                self.records(pay_receip_temp,pay_receip,total_amount,"sub",date,vouch_no,invoice_no,cheque_no,exp_type,account,acc_recev,description,amount,amountiw,tax_percent,tax_amount,total_amount)
                
                #for client record 
                self.client_record(client_temp,customers,amount,acc_recev,"payment",date,vouch_no,invoice_no,exp_type,account,cheque_no,description,amountiw,tax_percent,tax_amount,total_amount,"sub")

                self.client_record(invoice_balance,customers,amount,acc_recev,"purchase_invoice",date,vouch_no,invoice_no,exp_type,account,cheque_no,description,amountiw,tax_percent,tax_amount,total_amount,"sub")
                #for all bank record
                self.records(bank_temp,bank,total_amount,"sub",date,vouch_no,invoice_no,cheque_no,exp_type,account,acc_recev,description,amount,amountiw,tax_percent,tax_amount,total_amount)

                #for indivisual bank record
                self.ind_bank_record(bank_ind_temp,indvidual_bank,date,vouch_no,exp_type,account,cheque_no,acc_recev,description,amount,amountiw,tax_percent,tax_amount,total_amount,"sub")

                #for tax record
                self.records(tax_temp,tax,tax_amount,"add",date,vouch_no,invoice_no,cheque_no,exp_type,account,acc_recev,description,amount,amountiw,tax_percent,tax_amount,total_amount)

                #for head types
                self.head_record(head_temp,head_collection,total_amount,exp_type,date,vouch_no,account,description,amount,amountiw)
                if invoice_no != None:
                    for i in db['purchase_invoice'].find():
                        if i.get('voucher_no') == invoice_no:
    ...
    def generate_bank_receipt(self,window,receipt_temp,receipt,pay_receip,pay_receip_temp,customers,client_temp,bank,bank_temp,banks,bank_ind_temp,tax,tax_temp,invoice_balance,heads,company_name,user_name,db,invoice_temp,head_collection,head_temp):
        ...
        self.root.title("Generate Receipt")

        self.center_window(600,550)

        ttk.Label(self.root,text="Generate Bank Receipt Voucher",font=("helvetica",18,"bold")).pack(pady=30)
        ...
        ttk.Button(self.root,text="Generate" ,style="Module.TButton",cursor="hand2",width=20,command=lambda:generate(self.root,window,receipt_temp,receipt,pay_receip,pay_receip_temp,customers,client_temp,bank,bank_temp,banks,bank_ind_temp,tax,tax_temp,head_collection,head_temp)).pack(pady=10)    
        
        btn_frame = tk.Frame(self.root) 
        btn_frame.pack()

        ttk.Button(btn_frame,text="Back" ,style="Module.TButton",cursor="hand2",width=10,command=lambda:window(self.root,company_name,user_name)).grid(row=0,column=0,padx=5)
        ttk.Button(btn_frame,text="Exit" ,style="Module.TButton",cursor="hand2",width=10,command=self.root.destroy).grid(row=0,column=1,padx=5)
        ...
        def generate(root,window,receipt_temp,receipt,pay_receip,pay_receip_temp,customers,client_temp,bank,bank_temp,indvidual_bank,bank_ind_temp,tax,tax_temp,head_collection,head_temp):
            ...
            if not date or not vouch_no or not account or not acc_pay or not exp_type or not description or not amount or not tax_percent or not tax_amount or not total_amount:
                ...
                self.records(receipt_temp,receipt,total_amount,"add",date,vouch_no,invoice_no,cheque_no,exp_type,account,acc_pay,description,amount,amountiw,tax_percent,tax_amount,total_amount)
                #for overall bank and cash record
                self.records(pay_receip_temp,pay_receip,total_amount,"add",date,vouch_no,invoice_no,cheque_no,exp_type,account,acc_pay,description,amount,amountiw,tax_percent,tax_amount,total_amount)

                #for client record
                self.client_record(client_temp,customers,amount,acc_pay,"receipt",date,vouch_no,invoice_no,exp_type,account,cheque_no,description,amountiw,tax_percent,tax_amount,total_amount,"add")

                self.client_record(invoice_balance,customers,amount,acc_pay,"sale_invoice",date,vouch_no,invoice_no,exp_type,account,cheque_no,description,amountiw,tax_percent,tax_amount,total_amount,"add")
                
                #for all bank record
                self.records(bank_temp,bank,total_amount,"add",date,vouch_no,invoice_no,cheque_no,exp_type,account,acc_pay,description,amount,amountiw,tax_percent,tax_amount,total_amount)

                #for indivisual bank record
                self.ind_bank_record(bank_ind_temp,indvidual_bank,date,vouch_no,exp_type,account,cheque_no,acc_pay,description,amount,amountiw,tax_percent,tax_amount,total_amount,"add")
                    
                #for tax record
                self.records(tax_temp,tax,tax_amount,"add",date,vouch_no,invoice_no,cheque_no,exp_type,account,acc_pay,description,amount,amountiw,tax_percent,tax_amount,total_amount)
